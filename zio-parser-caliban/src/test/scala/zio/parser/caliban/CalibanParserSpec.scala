...
object CalibanParserSpec extends ZIOSpecDefault {
  ...
  def spec0: Spec[Any, ParsingError] = suite("CalibanParserSpec")(
    test("simple query with fields") {
      val query =
        """{
          ...
          |  }
          |}""".stripMargin
      CalibanParser.parseQuery(query).map { document =>
        assertTrue {
  ...
  def spec: Spec[Annotations, ParsingError] =
    suite("ParserSpec")(
      test("simple query with fields") {
        val query = """{
                      |  hero {
        ...
        CalibanParser.parseQuery(query).map { result =>
          assert(result)(
            equalTo(
              simpleQuery(
                selectionSet = List(
      ...
      test("invalid syntax") {
        val query = """{
                      ...
                      |  }
                      |}""".stripMargin.filter(_ != '\r')
        CalibanParser.parseQuery(query).exit.map { result =>
          assert(result)(
      ...
      test("type") {
        val gqltype =
          """type Hero {
            ...
            |powers: [String!]!
            |}""".stripMargin.filter(_ != '\r')
        CalibanParser.parseQuery(gqltype).map { result =>
          assert(result)(
            equalTo(
              Document(
                List(
      ...
      test("extend schema with directives") {
        ...
        CalibanParser.parseQuery(gqlSchemaExtension).map { result =>
          assert(result)(
            equalTo(
              Document(
                List(
      ...
      test("extend scalar with directives") {
        ...
        CalibanParser.parseQuery(gqlScalarExtension).map { result =>
          assert(result)(
            equalTo(
              Document(
                List(
      ...
      test("extend type with interfaces") {
        ...
        CalibanParser.parseQuery(gqlTypeExtension).map { result =>
          assert(result)(
            equalTo(
              Document(
                List(
      ...
      test("extend interface with directives") {
        ...
        CalibanParser.parseQuery(gqlInterfaceExtension).map { result =>
          assert(result)(
            equalTo(
              Document(
                List(
      ...
      test("extend union with directives") {
        ...
        CalibanParser.parseQuery(gqlUnionExtension).map { result =>
          assert(result)(
            equalTo(
              Document(
                List(
      ...
      test("extend enum with directives") {
        ...
        CalibanParser.parseQuery(gqlEnumExtension).map { result =>
          assert(result)(
            equalTo(
              Document(
                List(
      ...
      test("extend input with directives") {
        ...
        CalibanParser.parseQuery(gqlInputExtension).map { result =>
          assert(result)(
            equalTo(
              Document(
                List(
