...
import zio._
import zio.parser._
import zio.parser.presentation.CalibanParser.CalibanSyntax
...
object CalibanParser {
  ...
  lazy val comment: Syntax[String, Char, Char, Unit, Unit]                      =
    (Syntax.char('#') ~> Syntax.anyChar.repeatUntil(Syntax.char(LF) | Syntax.string(s"$CR$LF", ())).unit)
      .unit(Chunk.empty)
  lazy val whitespaceWithComment: Syntax[String, Char, Char, Unit, Unit]        =
    (whitespace <+> comment).*.unit(Chunk(Left(' ')))
  lazy val whitespaceWithComment1: Syntax[String, Char, Char, Unit, Unit]       =
    (whitespace <+> comment).+.unit(Chunk(Left(' ')))
